...
def build_similarity_graph(X, var=1.0, eps=0.0, k=0):
    ...
    if k != 0:
        ...
        """
        similarities=similarities-np.eye(n)
        
        for line in range(n):
            temp=list(similarities[line,:])
            for i in range(k):
                W[line,np.argmax(temp)]=max(temp)
                temp[np.argmax(temp)]=0
        
        #making the graph undirecetd by keeping the max of W and its transpose)
...
def how_to_choose_epsilon(gen_pam=0.0):
    """
    ...
    # gen_pam = 0.0 now passed as a parameter
    # to understand the meaning of the parameter, read worst_case_blob in generate_data.py
