...
def create_input_files(dataset,karpathy_json_path,captions_per_image, min_word_freq,output_folder,max_len=100):
    ...
    with open(os.path.join(output_folder,'train_flickr_imgid2idx.pkl'), 'rb') as j:
        train_data = pickle.load(j)
        
    with open(os.path.join(output_folder,'val_flickr_imgid2idx.pkl'), 'rb') as j:
